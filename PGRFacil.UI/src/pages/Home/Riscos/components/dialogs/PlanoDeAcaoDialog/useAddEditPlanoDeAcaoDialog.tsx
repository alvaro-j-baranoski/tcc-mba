...
export const useAddEditPlanoDeAcaoDialog = () => {
    ...
    const [responsavel, setResponsavel] = useState(isEdit ? modalState!.planoDeAcao!.responsavel : "");
    ...
    const [descricao, setDescricao] = useState(isEdit ? modalState!.planoDeAcao!.descricao : "");

    const queryClient = useQueryClient();

    const handleSuccess = () => {
        handleModal(false, null, null);
        invalidateQueriesForUpdatesOnRisco(queryClient, modalState!.risco!.gheId);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEdit) {
            editMutate({ gheId: modalState!.risco!.gheId, riscoId: modalState!.risco!.id, payload });
        } else {
            addMutate({ gheId: modalState!.risco!.gheId, riscoId: modalState!.risco!.id, payload });
        }
    };

    const handleDelete = () => {
        deleteMutate({ gheId: modalState!.risco!.gheId, riscoId: modalState!.risco!.id });
    };
